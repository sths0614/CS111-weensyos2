...
schedule(void)
{
  ...
  10009f:	a1 2c 76 10 00       	mov    0x10762c,%eax
  1000a4:	8b 10                	mov    (%eax),%edx

	if (scheduling_algorithm == 0)
  1000a6:	a1 30 76 10 00       	mov    0x107630,%eax
  ...
  1000bd:	83 b8 7c 6c 10 00 01 	cmpl   $0x1,0x106c7c(%eax)
  1000c4:	75 ee                	jne    1000b4 <schedule+0x18>
				run(&proc_array[pid]);
  1000c6:	83 ec 0c             	sub    $0xc,%esp
  1000c9:	05 34 6c 10 00       	add    $0x106c34,%eax
  1000ce:	50                   	push   %eax
...
interrupt(registers_t *reg)
{
  ...
  1000f6:	a1 2c 76 10 00       	mov    0x10762c,%eax
...
interrupt(registers_t *reg)
{
  ...
  100134:	a1 2c 76 10 00       	mov    0x10762c,%eax
  ...
  10014e:	ff 35 2c 76 10 00    	pushl  0x10762c
...
start(void)
{
  10016f:	53                   	push   %ebx
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
		proc_array[i].p_state = P_EMPTY;
  100170:	bb 84 6c 10 00       	mov    $0x106c84,%ebx
start(void)
{
	int i;
  ...
  100193:	68 34 6c 10 00       	push   $0x106c34
  ...
  10019d:	83 c4 10             	add    $0x10,%esp
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001a0:	c7 05 34 6c 10 00 00 	movl   $0x0,0x106c34
  1001a7:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001aa:	c7 05 7c 6c 10 00 00 	movl   $0x0,0x106c7c
  1001b1:	00 00 00 
	console_clear();
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001b4:	c7 05 84 6c 10 00 01 	movl   $0x1,0x106c84
  1001bb:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001be:	c7 05 cc 6c 10 00 00 	movl   $0x0,0x106ccc
  1001c5:	00 00 00 
	console_clear();
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001c8:	c7 05 d4 6c 10 00 02 	movl   $0x2,0x106cd4
  1001cf:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001d2:	c7 05 1c 6d 10 00 00 	movl   $0x0,0x106d1c
  1001d9:	00 00 00 
	console_clear();
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001dc:	c7 05 24 6d 10 00 03 	movl   $0x3,0x106d24
  1001e3:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001e6:	c7 05 6c 6d 10 00 00 	movl   $0x0,0x106d6c
  1001ed:	00 00 00 
	console_clear();
	...
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  1001f0:	c7 05 74 6d 10 00 04 	movl   $0x4,0x106d74
  1001f7:	00 00 00 
		proc_array[i].p_state = P_EMPTY;
  1001fa:	c7 05 bc 6d 10 00 00 	movl   $0x0,0x106dbc
  100201:	00 00 00 
	for (i = 1; i < NPROCS; i++) {
  ...
  100238:	68 84 6c 10 00       	push   $0x106c84
		proc->p_state = P_RUNNABLE;
	}
  ...
  100244:	80 0b 00 
	...
	// Initialize the scheduling algorithm.
	scheduling_algorithm = 0;
  100247:	c7 05 30 76 10 00 00 	movl   $0x0,0x107630
  10024e:	00 00 00 
...
segments_init(void)
{
  ...
  100258:	b8 c4 6d 10 00       	mov    $0x106dc4,%eax
...
segments_init(void)
{
  ...
  100279:	88 15 3c 10 10 00    	mov    %dl,0x10103c
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  10027f:	ba 2c 6e 10 00       	mov    $0x106e2c,%edx
segments_init(void)
{
	int i;
...
segments_init(void)
{
  ...
  1002a2:	c7 05 c8 6d 10 00 00 	movl   $0x180000,0x106dc8
  1002a9:	00 18 00 
	kernel_task_descriptor.ts_ss0 = SEGSEL_KERN_DATA;
  1002ac:	66 c7 05 cc 6d 10 00 	movw   $0x10,0x106dcc
  1002b3:	10 00 
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  1002b5:	66 89 0c c5 2c 6e 10 	mov    %cx,0x106e2c(,%eax,8)
  1002bc:	00 
  ...
  1002db:	b8 1a 00 10 00       	mov    $0x10001a,%eax
	...
	for (i = INT_SYS_YIELD; i < INT_SYS_YIELD + 10; i++)
		SETGATE(interrupt_descriptors[i], 0,
  1002e0:	ba 2c 6e 10 00       	mov    $0x106e2c,%edx
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
			SEGSEL_KERN_CODE, default_int_handler, 0);
	...
	// The clock interrupt gets special handling
	SETGATE(interrupt_descriptors[INT_CLOCK], 0,
  1002e5:	66 a3 2c 6f 10 00    	mov    %ax,0x106f2c
  1002eb:	c1 e8 10             	shr    $0x10,%eax
  1002ee:	66 a3 32 6f 10 00    	mov    %ax,0x106f32
  1002f4:	b8 30 00 00 00       	mov    $0x30,%eax
  1002f9:	66 c7 05 2e 6f 10 00 	movw   $0x8,0x106f2e
  100300:	08 00 
  100302:	c6 05 30 6f 10 00 00 	movb   $0x0,0x106f30
  100309:	c6 05 31 6f 10 00 8e 	movb   $0x8e,0x106f31
  ...
  10031e:	66 89 0c c5 2c 6e 10 	mov    %cx,0x106e2c(,%eax,8)
  100325:	00 
...
run(process_t *proc)
{
  100479:	8b 44 24 04          	mov    0x4(%esp),%eax
	current = proc;
  10047d:	a3 2c 76 10 00       	mov    %eax,0x10762c
